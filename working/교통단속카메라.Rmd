...
ggmap(krMap) + geom_point(data = cam_dj, aes(x=경도,y=위도,colour=시군구명))
```
---설치연도
```{r}
...
---대전광역시 관내 초중고의 학교 위치를 지도에 표시(점의 색은 "구분"으로 표시)
```{r}
...
---주어진 대전광역시 행정동 자료(지도)와 병원의 수 자료를 이용하여 그림을 그리시오.
###대전광역시 행정동 자료
```{r}
